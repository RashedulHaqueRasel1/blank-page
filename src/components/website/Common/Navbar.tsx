...
import { 
  ...
  EyeOff, Eye, X, Plus, FileText, Settings, Trash2 
} from "lucide-react";
...
// --- DATABASE HELPERS (OUTSIDE COMPONENT) ---
...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (key === "escape") { setShowDropdown(false); setShowSidebar(false); }
    };
  ...
  }, [theme, showCounter, showThemeIcon, currentFont, showSidebar, activeDocId]);
      ...
      <div ref={sidebarRef} className={`fixed top-0 left-0 h-full w-[280px] bg-[var(--editor-bg)] border-r border-[var(--border-color)] z-[100] transform transition-transform duration-300 ease-out shadow-2xl ${showSidebar ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full p-6">
          ...
          <button 
            ...
            className="flex items-center justify-center gap-3 w-full py-3.5 mb-10 rounded-xl border border-black/5 dark:border-white/5 bg-black/[0.03] dark:bg-white/[0.03] text-[14px] font-semibold text-black dark:text-white transition-all hover:bg-black/[0.08] dark:hover:bg-white/[0.08] active:scale-[0.98] cursor-pointer group"
          >
          ...
          <div className="flex flex-col gap-1 overflow-y-auto no-scrollbar flex-1 pr-2">
            ...
            {documents.map((doc) => (
              <div key={doc.id} className="group relative flex items-center mb-1 cursor-pointer">
                <button 
                  onClick={() => handleSwitchDoc(doc.id)} 
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl text-[14px] transition-all text-left flex-1 min-w-0 pr-12 cursor-pointer ${activeDocId === doc.id ? 'bg-black/5 dark:bg-white/5 text-black dark:text-white font-medium' : 'text-[#666] hover:bg-black/[0.02] dark:hover:bg-white/[0.02] hover:text-black dark:hover:text-white'}`}
                >
                  <FileText size={16} className={`shrink-0 ${activeDocId === doc.id ? 'opacity-100 text-black dark:text-white' : 'opacity-30 group-hover:opacity-100'}`} />
                  <span className="truncate flex-1 overflow-hidden whitespace-nowrap">{doc.title}</span>
                  {activeDocId === doc.id && (
                    <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-6 bg-black dark:bg-white rounded-r-full" />
                  )}
                </button>
                <button 
                  onClick={(e) => handleDeleteDoc(e, doc.id)}
                  className="absolute right-2 p-2 text-[#666] opacity-0 group-hover:opacity-100 hover:text-red-500 transition-all rounded-lg hover:bg-red-50 dark:hover:bg-red-950/20 z-10 cursor-pointer"
                >
                  <Trash2 size={14} />
                </button>
              </div>
          ...
          <div className="pt-6 border-t border-[var(--border-color)]">
            <button className="flex items-center gap-4 px-4 py-2 rounded-lg hover:bg-[#f5f5f5] dark:hover:bg-[#111] text-[14px] text-[#666] transition-colors w-full text-left">
              <Settings size={18} className="opacity-40" /> Settings
            </button>
          </div>
        </div>
      </div>

      <nav className="w-full h-14 md:bg-transparent bg-[var(--navbar-bg)] border-b border-transparent md:border-none border-[var(--border-color)] flex items-center justify-between px-6 fixed top-0 left-0 z-50 transition-all duration-300 md:pointer-events-none">
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              {showDropdown && (
                <div className="absolute right-0 mt-2 w-52 bg-[var(--navbar-bg)] border border-[var(--border-color)] rounded-xl shadow-2xl py-1.5 z-[60] overflow-hidden animate-in fade-in zoom-in duration-200">
                  ...
                  <button onClick={() => setMenuView(menuView === "fonts" ? "main" : "fonts")} className="w-full text-left px-4 py-2 text-[13px] text-[var(--editor-text)] hover:bg-[#f5f5f5] dark:hover:bg-[#222] flex items-center group">
                    <div className="flex items-center gap-3"><Type size={15} /> Font style</div>
                  </button>
                  <div className="h-[1px] bg-[var(--border-color)] my-1" />
                  <button onClick={() => setShowCounter(!showCounter)} className="w-full text-left px-4 py-2 text-[13px] text-[var(--editor-text)] hover:bg-[#f5f5f5] dark:hover:bg-[#222] flex items-center group">
